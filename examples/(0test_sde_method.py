...
def parse_args():
    parser = argparse.ArgumentParser(description="SDE method test Benchmark")
    parser.add_argument(
        "--model",
        type=str,
...
def run_with_sde_cache(model, args):
    """Run the model with SDE Cache acceleration"""
    # 创建SDE Cache包装器
    from xfuser.model_executor.pipelines.todo_sde_pipeline import xFuserSDEPipelineWrapper  # 替换为实际模块名
    input_config = {
        "model_type": args.model_type,
...
def main():
    ...
    for i in range(args.repeat):
        ...
        # 6. 运行SDE Cache（新增）
        if args.model_type == "flux":
            print("\nRunning with SDE Cache...")
            result, elapsed = run_with_sde_cache(model, args)
            if result is not None:
                results["SDECache"] = result
                times["SDECache"].append(elapsed)
                print(f"SDE Cache completed in {elapsed:.4f}s")
            torch.cuda.empty_cache()
